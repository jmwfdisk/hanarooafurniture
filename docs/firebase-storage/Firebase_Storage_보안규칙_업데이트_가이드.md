...
4. **`as-result-files/`** - 임직원 A/S 처리결과 게시판 첨부 파일
   - 읽기: 인증된 사용자만
   - 쓰기: 임직원(또는 관리자)만 (최대 10MB)

5. **`schedule-images/`** - 납품 일정관리 첨부 이미지
   - 읽기: 인증된 사용자만
   - 쓰기: 임직원(또는 관리자)만 (최대 10MB)
...
> ⚠️ **주의:** Storage 규칙은 `service firebase.storage`로 시작합니다.
> Firestore 규칙(`service cloud.firestore`)을 여기에 붙여넣으면 안 됩니다 (그건 Firestore Database → 규칙에 들어갑니다).
> 또한 Storage 규칙에서는 `get(/databases/$(database)/...)` 문법이 동작하지 않습니다.
> 역할/권한별 세부 제어는 앱 UI와 Firestore 규칙에서 처리하므로, Storage는 "로그인 사용자 + 용량 제한"으로 단순하게 둡니다.
...
match /as-result-files/{fileName} {
  ...
  allow write: if request.auth != null && 
                  ...
                  // 임직원 또는 관리자만 쓰기 가능, 파일당 최대 10MB
                  // (staff.html의 A/S 처리결과 게시판 첨부파일 업로드 경로)
}
```
