...
This single large file (~13k lines, several inline `<script>` blocks) holds all employee tools:
- **운영관리 통합 셸** (`#ops-modal`, z 2600): the nav no longer has separate 자재관리/재고현황/일정관리 links — one **운영관리** link opens a full-screen shell with a left sidebar (mobile: title row + horizontal chip row) of 인사관리/일정관리/자재관리/재고현황. **인사관리** is a segment panel (인사기록부/재직증명서 발급/연월차일정관리/급여지급명세서): 인사기록부 is real (`hrRecords/{uid}` — 임직원은 본인 기록 읽기전용, `hrCanManage()`(admin or 명시적 `permissions.hr`, 'all' 미포함) 데이터관리자·관리자는 대상 임직원 select 후 작성·수정·삭제; `hr*` functions in the ops shell script block). **연월차일정관리 is real too** (`hrOpenLeave`/`hrLeaveLoadAndRender`): 직원게시판 연월차일정(`leaveSchedule`)을 산정 기간으로 조회해 본인(데이터관리자는 대상 선택) 사용 내역을 집계 — 발생 연차는 근로기준법 제60조(`hrLeaveEntitlement`: 1년 미만 월 1일·최대 11일 / 15일 / 3년 이상 2년당 +1·최대 25일, 입사일은 hrRecords.joinDate), **사용 일수는 해당 월의 결재 도장(`appConfig/leaveApprovals`, 그룹별 hq/factory — `hrLeaveStampOf`)이 있어야 반영**, 미서명 달은 '결재 대기'로 비차감 표시; 연차·휴가만 차감(병가·경조사·기타 비차감), 문서 매칭은 author/createdBy 이름 기준(`hrLeaveIsOwn`). **급여지급명세서 is real** (`hrOpenPay`/`payExcelPicked`/`payGenerateAndUpload`): 데이터관리자가 급여 엑셀(.xlsx, 제목 'YYYY年 M月'·'지급일' 행·'소속/성명' 블록 반복 양식)을 업로드하면 SheetJS로 블록별 파싱 → 성명(동명이인은 직위) 매칭(미매칭은 미리보기에서 수동 선택/제외) → html2canvas+jsPDF(jsDelivr, 지연 로드)로 명세서 PDF 생성 → Storage `pay-slips/{ym}_{uid}.pdf` 업로드 → `paySlips/{uid}.months[ym]` set merge. 열람은 본인 목록(`payRenderMyList`)만. 재직증명서 발급 segment is still a static mockup (`opsHrMock`). `openOpsModal()` reparents the three existing tool modals into `#ops-tool-host` (once, they stay there); scoped CSS `.ops-tool-host .…-modal { position:static }` turns them into fill-height panels and hides their own 닫기/backdrop. `opsSelect(key)` closes the previous tool via its own close fn (listener/table teardown) then calls the tool's open fn — tool internals unchanged; it re-locks `body{overflow:hidden}` after each switch because tool close fns unlock it. Tools' auxiliary overlays (색상표 z4200, ctx menus z4000, staff confirm z100000) sit above the shell. Last-selected tool is remembered per page load (`opsCurrent`; default 일정관리).
