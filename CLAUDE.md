...
| `paySlips/{uid}` | staff 운영관리 인사관리(급여지급명세서) | read=**본인만**(`isOwner` — 데이터관리자·관리자도 타인 열람 불가, 쓰기 전용 업로드); write=`payManager()` = admin/`hr`/명시적 `pay`(급여명세 업로드 전용 권한, 'all' 미포함; client `payCanManage()` — `pay` 권한자는 `users` read 허용, `hrRecords`는 불가). `months['YYYY-MM_회사키']={url,payDate,company,...}`(구버전 키 `YYYY-MM`=하나로) — PDF는 Storage `pay-slips/`(read=로그인, 파일명 무작위 토큰이라 경로 추측 불가) |
