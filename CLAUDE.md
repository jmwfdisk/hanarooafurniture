...
New registrations are created with `status: 'pending'` and require admin approval before login works. Employee buttons (`#employee-button`, `.employee-button`) are `disabled` for non-employees. The optional `org` (소속) and `position` (직급 / 직책) fields are collected at signup and shown/edited in the member table (`position` is **required for 일반회원**, optional for 임직원; admin-editable inline via `editMemberPosition`/`saveMemberPosition`, mirroring `org`). **The register form HTML is duplicated static markup across ~13 pages, so `auth.js`'s `setupRegisterForm()` (called from `showAuthTab('register')`) restructures it at runtime in ONE place** — injects the `#reg-position` input above 이름, removes the legacy `#reg-user-type` dropdown, and replaces the single 회원가입 button with three: **본사 임직원 가입신청** → `register('employee-hq')` (saves `userType:'employee'` + `empGroup:'hq'`), **임직원 가입신청** → `register('employee')`, **일반회원 가입신청** → `register('general')` (`register(userTypeArg)` takes the type from the button, falling back to the dropdown then `'general'`). Stale-cached old `auth.js` degrades to the old form, which still works; `auth.js` includes are now cache-busted with `?v=YYYYMMDD` on all pages (bump on every auth.js edit). The 회원관리 유형 select has the same three options (`changeMemberType` maps `employee-hq` → `userType:'employee'`+`empGroup:'hq'`). `position` is an extra field on the `users` doc (no Firestore-rule allowlist, like `org`).
