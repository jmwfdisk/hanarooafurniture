...
- **운영관리 통합 셸** (`#ops-modal`, z 2600): the nav no longer has separate 자재관리/재고현황/일정관리 links — one **운영관리** link opens a full-screen shell with a left sidebar (mobile: title row + horizontal chip row) of 인사관리/일정관리/자재관리/재고현황. **인사관리** is a segment panel (인사기록부/재직증명서 발급/연월차일정관리/급여지급명세서): 인사기록부 is real (`hrRecords/{uid}` — 임직원은 본인 기록 읽기전용, `hrCanManage()`(admin or 명시적 `permissions.hr`, 'all' 미포함) 데이터관리자·관리자는 대상 임직원 select 후 작성·수정·삭제; `hr*` functions in the ops shell script block), the other three segments are static mockups (`opsHrMock`). `openOpsModal()` reparents the three existing tool modals into `#ops-tool-host` (once, they stay there); scoped CSS `.ops-tool-host .…-modal { position:static }` turns them into fill-height panels and hides their own 닫기/backdrop. `opsSelect(key)` closes the previous tool via its own close fn (listener/table teardown) then calls the tool's open fn — tool internals unchanged; it re-locks `body{overflow:hidden}` after each switch because tool close fns unlock it. Tools' auxiliary overlays (색상표 z4200, ctx menus z4000, staff confirm z100000) sit above the shell. Last-selected tool is remembered per page load (`opsCurrent`; default 일정관리).
